...
class ExcelImportService {
    ...
    async import(data) {
        ...
        const dates = data.tournaments.map((t) => this.parseDate(t.date)).filter(Boolean);
        if (dates.length === 0)
            throw new Error('Нет валидных дат турниров');
        ...
        await this.leaderboardService.getOrCreateLeaderboard(series.name, 'TOURNAMENT_SERIES', periodStart, periodEnd, series.id);
        // 4. Создать турниры
        ...
        for (const t of data.tournaments) {
            ...
            const toCardKey = (c) => String(c ?? '').trim();
            const seen = new Set();
