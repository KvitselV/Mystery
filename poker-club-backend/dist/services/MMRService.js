...
class MMRService {
     ...
     */
    calculateMMRChange(finishPosition, totalPlayers, isFinalTable) {
        let mmrChange = 0;
     ...
     */
    async updatePlayerMMR(playerProfileId, tournamentId) {
        ...
        const mmrChange = this.calculateMMRChange(result.finishPosition, totalPlayers, result.isFinalTable);
        ...
        console.log(`✅ Updated MMR for player ${playerProfileId}: ${previousMMR} → ${newMMR} (${player.rankCode})`);
        return updatedPlayer;
    }
    /**
     * Пересчитать ММР для всех игроков турнира
     */
    async recalculateTournamentMMR(tournamentId) {
        ...
        const results = allResults.filter((r) => r.tournament && r.tournament.id === tournamentId);
        console.log(`🔄 Recalculating MMR for ${results.length} players in tournament ${tournamentId}`);
        if (results.length === 0) {
        ...
        for (const result of results) {
            try {
                ...
                console.error(`❌ Failed to update MMR for player ${result.player.id}:`, error instanceof Error ? error.message : String(error));
            }
        }
     ...
     */
    async getTopPlayersByMMR(limit = 50) {
     ...
     */
    async getPlayersByRank(rankCode) {
        return this.playerRepository.find({
            where: { rankCode },
            order: { mmrValue: 'DESC' },
            relations: ['user'],
